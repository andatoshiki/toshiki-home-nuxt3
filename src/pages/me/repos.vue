...
export default Vue.extend({
    ...
    fetchOnServer: false,
    async fetch() {
        const filter = ['andatoshiki', 'toshiki', 'andatoshiki.github.io', 'dev', 'toshikidev', 'shikiology']

        const repos: Repository[] = (
            await this.$axios.get('https://gh-api.toshiki.dev/users/andatoshiki/repos?per_page=100')
        ).data
...
<template>
    <PageLayout title="Repositories" description="My public projects on GitHub." class="margin">
        ...
        <div v-else class="grid gap-4 md:grid-cols-2">
            <SmartLink v-for="(repo, index) in repos" :key="`repo-${index}`" :href="repo.html_url" blank>
                <CardRepository
                    :name="repo.name"
