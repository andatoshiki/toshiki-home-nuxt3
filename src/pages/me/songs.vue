...
<template>
    <PageLayout
        title="Songs"
        description="My latest music activity powered by Last FM. Check out the Daily Songs page for a fresh new recommendation!"
        class="space-y-12 margin"
    >
        <LoadersSongs v-if="$fetchState.pending === true || $fetchState.error !== null" />
                ...
                <div class="grid gap-x-0 gap-y-4 md:gap-x-12 md:grid-cols-2">
                    ...
                    <div class="flex space-x-4 items-center justify-between">
                        ...
                        <div class="flex space-x-2 items-center">
                            <SmartLink href="https://last.fm/user/andatoshiki" class="flex-shrink-0" blank
                                >@{{ lastFm.user.name }}</SmartLink
                            >

                            <SmartImage :src="lastFm.user.image" class="rounded-full h-6 w-6" />
                        </div>
                    </div>
                    ...
                    <div class="flex space-x-4 items-center justify-between">
                        ...
                        <div class="flex space-x-2 items-center">
                            <div class="truncate">
                                {{ new Date().getFullYear() - new Date(lastFm.user.registered * 1000).getFullYear() }}
