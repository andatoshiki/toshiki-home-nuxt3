...
/* Import types */
import type { LanyardData, Activity } from '@andatoshiki/vue-lanyard/@types/lanyard'
...
export default Vue.extend({
    ...
    async mounted() {
        // Connect to Lanyard Socket API, send heartbeat every 30 seconds and replace the Vue data value with the message using @andatoshiki/vue-lanyard module
        this.socket = (await this.$lanyard({
            userId: '988794736435408916',
            socket: true
        })) as WebSocket
...
<template>
    <div
        v-if="finished === false || !getStatusDetails || Object.keys(lanyard).length === 0"
